...
def Saver(preds, gt, name, save_folder, config):
    ...
    for i, ri in enumerate(rs):
        ...
        for j in range(64):
            r = ri[0, j]
            att_i_path = os.path.join(save_folder, 'atten', str(i), str(j))
            check_path(att_i_path)
            att_path = os.path.join(att_i_path, name + '.png')
            
            im_depth = r.cpu().detach().numpy()
            im_depth = (im_depth - np.min(im_depth)) / (np.max(im_depth) - np.min(im_depth))
            #print(np.max(im_depth), np.min(im_depth))
            im_color=cv2.applyColorMap(cv2.convertScaleAbs(im_depth,alpha=255),cv2.COLORMAP_JET)
            #convert to mat png
            Image.fromarray(im_color).save(att_path)
            #Image.fromarray(r.cpu().detach().numpy() * 255).convert('L').save(att_path)
'''
'''
def Saver(preds, gt, name, save_folder, config):
